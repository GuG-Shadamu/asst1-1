...
> The main reason is that vectorized code performance greatly depends on the distribution and nature of your data. The vector width, in SIMD context, defines how many data elements can be processed in parallel. If your VECTOR_WIDTH is large, that means you are trying to process a lot of elements simultaneously. However, there are certain conditions which may reduce the efficiency of your vectorized code.
> 1. Remainder Problem: If N, the number of data points, is not a multiple of VECTOR_WIDTH, you will have remainders that cannot be processed in a vectorized manner. In your code, you handled this by calling a serial version of the function for the remaining elements. As VECTOR_WIDTH increases, the probability of having a non-zero remainder also increases, hence you will have more elements processed in a serial manner, reducing the overall vector utilization.
> 2. Conditional Branching: Vectorized code usually struggles with conditional operations, especially in the case where the result of the condition varies within the data set being processed. In your code, there are loops and conditions that depend on the data, which may lead to something called "masking" or "predication". Not all lanes of the SIMD unit are active all the time, which lowers the utilization. For example, if exp_[i] <= 0 for some i, those lanes will be idle during the loop.
> 3. Memory Access Patterns: Vectorized code can benefit greatly from contiguous and aligned memory access, but as VECTOR_WIDTH increases, the data set you are trying to load/store at once also increases. If your data is not well aligned or if your data set is too large, you may encounter cache misses or other memory related problems that would reduce performance.
> 4. Hardware Limitations: Lastly, not all hardware supports all vector widths efficiently. The hardware is optimized to handle a certain vector width better. If VECTOR_WIDTH is too large, the hardware might not be able to process all elements at once, essentially breaking your larger width down to multiple smaller widths, which lowers performance.
> To conclude, vectorizing code requires a careful analysis of the data patterns and the hardware capabilities, and blindly increasing the vector width might not always increase performance. It's always important to do empirical performance testing on your target architecture.

1.  _Extra credit: (1 point)_ Implement a vectorized version of `arraySumSerial` in `arraySumVector`. Your implementation may assume that `VECTOR_WIDTH` is a factor of the input array size `N`. Whereas the serial implementation has `O(N)` span, your implementation should have at most `O(N / VECTOR_WIDTH + log2(VECTOR_WIDTH))` span.  You may find the `hadd` and `interleave` operations useful.
